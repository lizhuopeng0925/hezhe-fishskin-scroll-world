...
function mountScrollWorld(container, config) {
  ...
  SEGMENTS.forEach(s => {
    ...
    img.loading = s.kind === 'conn' ? 'eager' : 'lazy';
    const poster = (isMobile() && s.stillM) ? s.stillM : s.still;
    if (poster) img.src = poster;
    scene.appendChild(img); stage.appendChild(scene);
    s.el = scene; s.img = img; s.video = null; s.hasClip = false;
    s.loading = false; s.ready = false; s.requestedUrl = null; s.loadedUrl = null;
  ...
  function loadClip(s) {
    ...
    function attachVideoSource(src, objectUrl = null) {
      ...
      s.el.appendChild(v); s.video = v; s.hasClip = true; s.loadedUrl = url;
    }

    // Directly-opened file:// previews cannot fetch() local MP4 files because
    // of browser origin policy. A media element may load the same relative file
    // directly, while the eager connector poster remains a no-blank fallback.
    ...
    fetch(url).then(r => r.ok ? r.blob() : Promise.reject(new Error('404')))
      .then(blob => {
        // A breakpoint change may have selected another encode while this fetch
        // was in flight. attachVideoSource discards that stale result.
        const objectUrl = URL.createObjectURL(blob);
        attachVideoSource(objectUrl, objectUrl);
      }).catch(() => { s.loading = false; s.requestedUrl = null; });
  }

  function syncMediaVariant() {
    const mobile = isMobile();
    SEGMENTS.forEach(s => {
      const poster = (mobile && s.stillM) ? s.stillM : s.still;
      if (poster && s.img.getAttribute('src') !== poster) s.img.src = poster;

      const wantedUrl = (mobile && s.clipM) ? s.clipM : s.clip;
      const wrongLoadedClip = s.loadedUrl && s.loadedUrl !== wantedUrl;
      const wrongPendingClip = s.loading && s.requestedUrl && s.requestedUrl !== wantedUrl;
      if (!wrongLoadedClip && !wrongPendingClip) return;

      // The viewport crossed the desktop/mobile breakpoint after this clip was
      // loaded. Remove the mismatched encode; read() will load the right one for
      // nearby segments immediately after layout finishes.
      ++s.loadToken;
      if (s.video) s.video.remove();
      if (s.objectUrl) URL.revokeObjectURL(s.objectUrl);
      s.video = null; s.hasClip = false; s.loading = false; s.ready = false;
      s.requestedUrl = null; s.loadedUrl = null; s.objectUrl = null;
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      if (!s.hasClip || !s.ready) {
        // The first two portrait exhibits already fill a phone viewport. Keep
        // their still-image drift subtle so the garment and both trouser legs
        // remain visible instead of being enlarged out of frame.
        const protectOpeningExhibit = isMobile() && s.kind === 'dive' && s.si < 2;
        const sc = reduce ? 1 : (protectOpeningExhibit ? 1 + local * 0.035 : 1.03 + local * 0.14);
  ...
  function raf() {
    const eps = isMobile() ? 0.006 : 0.003;
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      if (!s.hasClip || !s.ready || !s.video) continue;
      // Never queue a seek while the decoder is resolving the last one. A short
      // critically damped catch-up bridges wheel-event gaps without turning the
      // clip into independently playing video; it settles in roughly 0.1 seconds.
      if (s.video.seeking) continue;
      if (!s.visible && Math.abs(s.cur - s.target) < 0.002) continue;
      const delta = s.target - s.cur;
      s.cur = Math.abs(delta) < 0.002 ? s.target : s.cur + delta * 0.62;
